...
##### Matrix permeability ----
# best-ranked model: asymptotic 
asymptotic <- drm(habitat_configuration_simulations$assess_top_sections ~ habitat_configuration_simulations$matrix_permeability, fct = DRC.asymReg())
x_values <- 10:max(habitat_configuration_simulations$matrix_permeability)
y_values <- coefficients(asymptotic)[3] - (coefficients(asymptotic)[3] - coefficients(asymptotic)[1]) * exp (- coefficients(asymptotic)[2] * x_values)
(aggreg_perm_config <- ggplot(habitat_configuration_simulations, aes(x = matrix_permeability, y = assess_top_sections)) +
    annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.25, ymax = 1, alpha = 0.15, fill = "grey15") +
    annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.5, ymax = 1, alpha = 0.15, fill = "grey15") +
    annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.75, ymax = 1, alpha = 0.15, fill = "grey15") +
    geom_point(size = 0.05) +
...
##### Perceptual range ----
# best-ranked model: exponential 
exponential_growth <- drm(habitat_configuration_simulations$assess_top_sections ~ habitat_configuration_simulations$perceptual_range, fct = DRC.expoGrowth())
x_values <- 5:max(habitat_configuration_simulations$perceptual_range)
y_values <- coefficients(exponential_growth)[1] * exp(coefficients(exponential_growth)[2] * x_values)
(aggreg_percep_config <- ggplot(data = habitat_configuration_simulations, aes(x = perceptual_range, y = assess_top_sections)) +
    annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.25, ymax = 1, alpha = 0.15, fill = "grey15") +
    annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.5, ymax = 1, alpha = 0.15, fill = "grey15") +
    annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.75, ymax = 1, alpha = 0.15, fill = "grey15") +
    geom_point(size = 0.05) +
...
(aggreg_vision_config <- ggplot(data = habitat_configuration_simulations, aes(x = vision_angle, y = assess_top_sections)) +
   annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.25, ymax = 1, alpha = 0.15, fill = "grey15") +
   annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.5, ymax = 1, alpha = 0.15, fill = "grey15") +
   annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 0.75, ymax = 1, alpha = 0.15, fill = "grey15") +
   geom_point(size = 0.05) +
...
#### Total crossings ----
##### Scenarios ----
(crossings_scenarios_config <- ggplot(habitat_configuration_simulations, aes(x = factor(scenario), y = total_crossings)) +
   ggdist::stat_halfeye(adjust = 0.5, width = 0.75, justification = -0.25, .width = 0, point_colour = NA, fill = "black") +
   geom_boxplot(width = 0.15, outlier.color = NA, alpha = 0.5) +
...
##### Matrix permeability ----
# best-ranked model: asymptotic
asymptotic <- drm(habitat_configuration_simulations$total_crossings ~ habitat_configuration_simulations$matrix_permeability, fct = DRC.asymReg())
x_values <- 10:max(habitat_configuration_simulations$matrix_permeability)
y_values <- coefficients(asymptotic)[3] - (coefficients(asymptotic)[3] - coefficients(asymptotic)[1]) * exp (- coefficients(asymptotic)[2] * x_values)
(crossings_perm_config <- ggplot(data = habitat_configuration_simulations, aes(x = matrix_permeability, y = total_crossings)) +
    geom_point(size = 0.05) +
    geom_line(data = data.frame(x = x_values, y = y_values), aes(x, y), 
              col = "red", linewidth = 0.75) +
...
##### Perceptual range ----
# best-ranked model: exponential 
exponential_growth <- drm(habitat_configuration_simulations$total_crossings ~ habitat_configuration_simulations$perceptual_range, fct = DRC.expoGrowth())
x_values <- 5:max(habitat_configuration_simulations$perceptual_range)
y_values <- coefficients(exponential_growth)[1] * exp(coefficients(exponential_growth)[2] * x_values)
(crossings_percep_config <- ggplot(data = habitat_configuration_simulations, aes(x = perceptual_range, y = total_crossings)) +
    geom_point(size = 0.05) +
    geom_line(data = data.frame(x = x_values, y = y_values), aes(x, y),
              col = "red", linewidth = 0.75) +
...
##### Vision angle ----
# best-ranked model: asymptotic
asymptotic <- drm(habitat_configuration_simulations$total_crossings ~ habitat_configuration_simulations$vision_angle, fct = DRC.asymReg())
x_values <- 90:max(habitat_configuration_simulations$vision_angle)
y_values <- coefficients(asymptotic)[3] - (coefficients(asymptotic)[3] - coefficients(asymptotic)[1]) * exp (- coefficients(asymptotic)[2] * x_values)
(crossings_vision_config <- ggplot(data = habitat_configuration_simulations, aes(x = vision_angle, y = total_crossings)) +
    geom_point(size = 0.05) +
    geom_line(data = data.frame(x = x_values, y = y_values), aes(x, y), 
              col = "red", linewidth = 0.75) +
...
(config_aggreg <- aggreg_scenarios_config + aggreg_perm_config + aggreg_percep_config + aggreg_vision_config +
    patchwork::plot_layout(ncol = 4) + 
    patchwork::plot_annotation(title = 'Habitat configuration model', theme = theme(plot.title = element_text(size = 12, vjust = 1))))
