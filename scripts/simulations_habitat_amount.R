...
# define model parameters values
...
# run all simulations
results_crossings <- progressr::with_progress(nlrx::run_nl_all(nl = nl_crossings, split = 4))

# attach results of the simulations to netlogo object
setsim(nl_crossings, "simoutput") <- results_crossings

# save object
saveRDS(nl_crossings, here::here("results", "habitat_amount_simulations.rds"))
