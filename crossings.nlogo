...
to setup_turtles
  create-turtles number_of_individuals [set color black
    set size 1]
...
to move
  ask turtles [check_move
    if chosen_patch = nobody [rt 180]
    if chosen_patch != nobody [face chosen_patch fd 1]
   ask patch-here [set visits visits + 1]
  ]
end

to check_move
  let candidate_patches patches in-cone perceptual_range vision_angle
  if (count candidate_patches = 1) [
   ask turtles-on candidate_patches [face patch (max-pxcor / 2) (max-pycor / 2)]
    let temporary_patches patches in-cone perceptual_range 120
...
to-report assess_top_sections
  let sort_crossings sort-by > [visits] of patches with [habitat = 0]
  let all_crossings sum sort_crossings

  ifelse total_crossings > 0 [
  let top_sections_effectiveness sum (sublist sort_crossings 0 (max-pycor * .25)) ; QUE ESTA ACONTECENDO
  let effectiveness top_sections_effectiveness / total_crossings
    report precision effectiveness 3]
  [report 0]
end

to-report total_crossings
