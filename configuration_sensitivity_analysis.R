...
ggplot(assess_graph, aes(x = original, y = parameter,  group = index)) +
  ...
  scale_y_discrete(limits = c("perceptual_range", "vision_angle", "matrix_permeability", "scenarios"),
                   labels=c("vision_angle" = "Vision angle", "scenarios" = "Scenarios",
...
ggplot(crossings_graph, aes(x = original, y = parameter,  group = index)) +
  geom_pointrange(aes(xmin = `min. c.i.`, xmax = `max. c.i.`, color = index), 
                  position = position_dodge(width = 0.3), size = 0.3) +
  scale_colour_manual(values = c("first-order" = "black", "total" = "grey50"),
                      labels = c("First-order index", "Total index"), name = " ") +
  labs(title = "Total crossings", y = "Parameter", x = "Sobol Index") +
  scale_y_discrete(limits = c("matrix_permeability", "vision_angle", "scenarios", "perceptual_range"),
                  labels=c("vision_angle" = "Vision angle", "scenarios" = "Scenarios",
                          "perceptual_range" = "Perceptual range", "matrix_permeability" = "Matrix permeability")) +
  theme(legend.position = "bottom") 
